...
function Export-NewRelicServerMetics {
  ...
  #>
  [CmdletBinding()]
  param($NRProcessMetrics,$NRheaders,$NRFromDateTime, $NRToDateTime,$NRPollPeriod,$NRhash= $Global:NRServerHash)
...
$Global:MetricCollection = New-Object System.Collections.ArrayList($null)
    ...
    $NRhash.GetEnumerator() | ForEach-Object {
        ...
        foreach ($Metric in $NRProcessMetrics) 
            ...
            $MetricsHash.add(($Metric+ "__CPU_Average(%)"),$average_value)
...
#    return $Global:NRServerIDs
$MetricCollection | Export-Csv -Path C:\temp\mytest1.csv
}
